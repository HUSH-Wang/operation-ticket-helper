...
**操作票辅助工具**是一个基于 Vue 3 + Vite 的单页面应用（SPA），专为电力系统运维场景设计。它提供三大核心功能模块：

- **操作票编辑**：提供沉浸式的操作步骤文本编辑，支持选中行一键转换状态、完整的撤销/重做记录等功能。
- **压板文本生成**：根据模板快速批量生成或逆向解析压板操作步骤文本。
- **步骤状态转换**：将已有操作步骤（无论来自哪种状态）智能转换为目标状态格式。
...
### 功能一：操作票编辑（TicketEditor）

- **沉浸式双向编辑**：专门设计的宽大编辑区，支持自由输入或无缝粘贴多行操作步骤信息。
- **原位快捷状态转换**：只需在编辑区选中目标行，利用顶部悬浮工具栏或右键菜单指定的“状态”，即可在**原位将该行替换为目标状态字句**，无需转移到别处再复制。并支持 `Alt + 1/2/3/4` 快捷键极速操作。
- **高级撤销与重做**：内建最高效的历史快照记录。不论是文本手动输入，还是复杂的正则快捷修改，均支持快捷键/右键一键完美还原，告别误操作困扰。
- **阅读环境定制**：贴心地配备了视图放大（`A+`）与缩小（`A-`）控制钮，可自由随心调整字体大小。
- **一键清空电压电流**：内置正则清理工具，点击按钮即可将括号内所有的数字与标点处理为标准的“空格+单位”格式（例如将 `(Ua:10kV)` 替换为 `（Ua：   kV）`），极大简化录入后排版工作。
- **防丢失机制**：自动每10秒在后台进行本地持久保存，意外刷新不丢数据。

---

### 功能二：压板文本生成（PlatGenerator）

#### 1.1 基础输入模式（正向生成）

- **操作类型切换**：提供 5 种操作类型单选按钮快速切换：
  - 投入 / 退出 / 检查投入 / 检查退出 / 测量后投入
- **保护屏名输入**：对应模板占位符 `{screen}`，支持一行文本填写；仅在“直接输入压板列表”模式参与生成
...
- 系统基于所有 5 条压板模板构建正则表达式，自动逆向提取纯压板名称
- 解析模式下“保护屏名 `{screen}`”输入框会被禁用但不会清空页面内容；系统会固定将 `{screen}` 按空字符串处理，切回“直接输入压板列表”模式后恢复可编辑并继续使用原输入内容
- 若模板中 `{screen}` 与 `{plat}` 连续出现（如 `退出{screen}{plat}压板`），解析模式会把整段中间文本都提取为 `{plat}`：
  - 输入 `退出500kVⅡ段母线第一套母差保护屏跳5023开关A相TC1跳闸出口1C2LP1压板` 时，严格语义上 `500kVⅡ段母线第一套母差保护屏` 是 `{screen}`、`跳5023开关A相TC1跳闸出口1C2LP1` 是 `{plat}`
  - 但在当前解析兼容策略下，系统会按 `{screen}=空字符串`、`{plat}=500kVⅡ段母线第一套母差保护屏跳5023开关A相TC1跳闸出口1C2LP1` 处理，用于兼容“不预先填写屏名也能解析并重新生成文本”的旧操作习惯。该行为目前能够正常工作，但并不代表严格的变量语义
...
| 占位符 | 含义 |
|--------|------|
| `{screen}` | 保护屏名（直接输入模式对应输入框内容；解析模式固定为空字符串） |
| `{plat}` | 逐行压板名称 |
...
测量后投入：测量{screen}{plat}压板两端对地确无异极性电压后，投入
```

---

### 功能三：任务状态转换（TaskConverter）
...
- **一键清空电压电流**：包含“清空电压电流”快捷功能，支持将文本括号内的测试数值一键替换为标准排版空格，并自动将中英文符号统一格式化。

#### 3.2 正则匹配引擎

- 使用**命名捕获组**正则自动提取设备名称（支持 `{deviceName}`、`{a}`、`{b}` 等任意命名占位符）
- 匹配前自动去除文本中所有空白字符，避免因格式差异导致匹配失败
- **针对中英文标点增强兼容**：底层智能过滤全半角括号 `()`/`（）`、中英文逗号冒号及顿号的差异，并支持灵活兼容如单位 `A` 前后是否有空格的缺陷输入，大幅提升了对杂乱复制文本的匹配成功率。
- `{n}` 为数值通配符，生成时渲染为4个空格（满足填票排版需要），解析时匹配 `\d*` 甚至小数及带空格的格式。
...
│   ├── assets/                   # 静态资产
│   ├── components/
│   │   ├── AboutPage.vue         # 网页说明及更新日志（聚合页）
│   │   ├── TicketEditor.vue      # 操作票编辑器（支持原位转换及撤销）
│   │   ├── PlatGenerator.vue     # 压板文本生成器
│   │   └── TaskConverter.vue     # 任务状态转换器
│   ├── templates/
│   │   ├── ticket_template.json  # 应用默认数据（platTemplates + stateNames + tasks）
│   │   └── plat_templates.json   # 压板模板的独立备份（仅包含 4 条模板）
│   ├── App.vue                   # 根组件，全局 Tab 布局 + 持久化数据统一调度
...
| 键名 | 存储内容 | 格式 |
|------|----------|------|
| `ticketEditorContent` | 编辑器中的临时内容缓存 | 纯文本 |
| `ticketPlatTemplates` | 压板模板配置 | `{ input, exit, checkInput, checkExit, measureInput }` |
