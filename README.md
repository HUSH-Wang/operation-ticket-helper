...
- 若模板中 `{screen}` 与 `{plat}` 连续出现（如 `退出{screen}{plat}压板`），解析模式会把“保护屏名 {screen}”输入框作为固定屏名参与匹配：
  - 当屏名输入框填写了 `500kVⅡ段母线第一套母差保护屏` 时，`退出500kVⅡ段母线第一套母差保护屏跳5023开关A相TC1跳闸出口1C2LP1压板` 会被解析为 `{screen}=500kVⅡ段母线第一套母差保护屏`、`{plat}=跳5023开关A相TC1跳闸出口1C2LP1`
  - 当屏名输入框留空时，系统会将 `{screen}` 按空字符串处理，整段中间文本会进入 `{plat}`，用于兼容“不预先填写屏名也能解析并重新生成文本”的旧操作习惯。该行为是当前兼容策略，并不代表严格的变量语义
